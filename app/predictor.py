...
class FeatureProcessor:
    ...
    def validate_features(self, features: Union[Dict, List, np.ndarray]) -> bool:
        ...
        if len(feature_array) != self.required_features:
            logger.warning(f"Expected {self.required_features} features, got {len(feature_array)}. Padding/truncating...")
    ...
    def _dict_to_array(self, feature_dict: Dict) -> np.ndarray:
        ...
        expected_features = [
            ...
            'transaction_weekend', 'is_business_hours', 'card_amount_mean',
            'card_txn_count_recent', 'time_since_last_txn', 'merchant_risk_score',
    ...
    def process_transaction_features(self, transaction_data: Dict) -> np.ndarray:
        ...
        features = {
            ...
            'is_business_hours': int(transaction_data.get('is_business_hours', 1 if 9 <= hour <= 17 else 0)),
...
class BatchPredictor:
    ...
    def predict_batch(self, transactions: List[Dict], model_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """
