...
class FraudAPITester:
    ...
    def test_single_prediction(self):
        ...
        try:
            response = self.session.post(f"{self.base_url}/predict", json=sample_transaction)
    ...
    def test_batch_prediction(self):
        ...
        try:
            response = self.session.post(f"{self.base_url}/batch_predict", json=batch_request)
            ...
            if response.status_code == 200:
                ...
                for i, pred in enumerate(data["predictions"][:3]):  # Show first 3
                    print(
                        f"     Transaction {i+1}: {pred['risk_level']} risk ({pred['fraud_probability']:.3f})"
                    )
