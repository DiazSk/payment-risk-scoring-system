...
class FraudModelEvaluator:
    ...
    def plot_precision_recall_curves(self, evaluation_results: Dict, figsize: Tuple = (10, 8)):
        ...
        if self.save_plots:
            plt.savefig(
                self.plots_dir / "precision_recall_curves.png", dpi=300, bbox_inches="tight"
            )
    ...
    def plot_feature_importance(
        ...
        if self.save_plots:
            plt.savefig(
                self.plots_dir / f"{model_name}_feature_importance.png",
                dpi=300,
                bbox_inches="tight",
            )
    ...
    def create_interactive_model_comparison(self, evaluation_results: Dict):
        ...
        fig.add_trace(
            go.Scatter(
                x=df_comparison["recall"],
                y=df_comparison["precision"],
                mode="markers+text",
                text=df_comparison["model"],
                textposition="top center",
                name="Models",
                marker=dict(
                    size=12,
                    color=df_comparison["f1_score"],
                    colorscale="Viridis",
                    showscale=True,
        ...
        # Update layout
        fig.update_layout(
            title_text="Fraud Detection Model Performance Dashboard", showlegend=False, height=800
        )
    ...
    def create_business_impact_visualization(
        ...
        for model_name, results in evaluation_results.items():
            ...
            fraud_prevented = (
                recall * annual_transactions * 0.002 * avg_fraud_amount
            )  # 0.2% fraud rate
        ...
        ax2.bar(
            x - width / 2,
            df_business["fraud_prevented"] / 1000,
            width,
        ...
        ax2.bar(
            x + width / 2,
            df_business["false_positive_cost"] / 1000,
            width,
        ...
        for i, model in enumerate(df_business["model"]):
            ax3.annotate(
                model,
                (df_business["fpr"].iloc[i], df_business["recall"].iloc[i]),
                xytext=(5, 5),
        ...
        efficiency_ratio = df_business["recall"] / (
            df_business["fpr"] + 0.001
        ...
        if self.save_plots:
            plt.savefig(
                self.plots_dir / "business_impact_analysis.png", dpi=300, bbox_inches="tight"
            )
    ...
    def generate_evaluation_report(
        self, evaluation_results: Dict, feature_names: List[str] = None
    ) -> str:
        ...
        best_f1_model = max(
            evaluation_results.keys(), key=lambda x: evaluation_results[x]["metrics"]["f1_score"]
        )
        best_recall_model = max(
            evaluation_results.keys(), key=lambda x: evaluation_results[x]["metrics"]["recall"]
        )
        ...
        if self.save_plots:
            with open(
                self.plots_dir.parent / "reports" / "detailed_evaluation_report.txt", "w"
            ) as f:
                f.write(report_text)
    ...
    def run_complete_evaluation(self, evaluation_results: Dict, feature_names: List[str] = None):
        ...
        return {"business_impact": business_impact, "evaluation_report": report}
