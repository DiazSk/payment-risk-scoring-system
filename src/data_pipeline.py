...
class DataPipeline:
    ...
    def generate_sample_data(
        self, n_samples: int = 10000, fraud_rate: float = 0.0017
    ) -> pd.DataFrame:
    ...
    def clean_data(self, df):
        """
        Clean the dataframe by removing nulls and duplicates
        This method wraps process_data for backward compatibility
        """
        # If the class has a process_data method, use it; otherwise, do basic cleaning
        process_data_method = getattr(self, "process_data", None)
        if callable(process_data_method):
            return process_data_method(df)
        df_clean = df.copy()
        # Remove nulls
