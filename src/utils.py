...
class ModelUtils:
    ...
    @staticmethod
    def save_model_metadata(metadata: Dict, filename: str = "model_metadata.json", model_dir: str = "models/"):
...
class DataUtils:
    ...
    @staticmethod
    def split_features_target(df: pd.DataFrame, target_col: str = 'isFraud') -> Tuple[pd.DataFrame, pd.Series]:
...
class MetricsUtils:
    ...
    @staticmethod
    def calculate_fraud_metrics(y_true: np.array, y_pred: np.array, y_prob: Optional[np.array] = None) -> Dict:
...
class VisualizationUtils:
    ...
    @staticmethod
    def plot_feature_correlation(df: pd.DataFrame, target_col: str = 'isFraud', top_n: int = 20) -> None:
...
def main():
    ...
    print(f"✅ Class imbalance analysis: {analysis['imbalance_ratio']:.1f}:1 ratio")
